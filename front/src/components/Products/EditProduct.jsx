...
const EditProduct = () => {
  ...
  //Función submit que inserta el producto modificado.
  const onSubmit = async (data, e) => {
    e.preventDefault();
    setError('');    
    try {
      data.valoracion = items[0].valoracion;
                ...
                return (
                  <div key={item.id_producto}>
                    ...
                    <div className='mb-4' key={item.urlfoto}>
                      ...
                      <input className='form-control my-2' readOnly="readonly" type="text" defaultValue={item.urlfoto} onChange={(e) => setUrlProducto(e.target.value)} {...register('urlfoto', {
                        required: true,
                        minLength: {
                          value: 4,
                        }
